import { getCulturalMomentSelectionContext, selectCulturalMomentForYear } from "../../src/server/cultural/selectCulturalMoment";
...
import type { CulturalMomentEntry } from "../../src/content/culturalMoments/types";
import { parseCsvList } from "../../src/lib/parseCsv";

type CulturalMomentPayload = {
  schema_version: "1.0";
  country_qid: string;
  year: number;
  lang: "fr" | "en";
...
function buildPayload(entry: CulturalMomentEntry, params: { year: number; countryQid: string; lang: "fr" | "en"; slot: number }): CulturalMomentPayload {
  return {
    schema_version: "1.0",
    ...
    slot: params.slot,
    moment_id: entry.id,
    moment_label: entry.label || entry.momentLabel,
...
export async function onRequestGet(context: { request: Request }): Promise<Response> {
  ...
  log("cultural_moment_served", { year, countryQid, lang, slot, momentId: entry.id });
