import { selectInventionsForYear } from "../../src/server/inventions/selectInvention";
...
import type { InventionEntry } from "../../src/content/inventions/types";
...
function buildPayload(entry: InventionEntry, params: { year: number; countryQid: string; lang: "fr" | "en"; slot: number }): InventionScenePayload {
  const matchedYear = entry.releaseYear;
  return {
    schema_version: "1.0",
    ...
    lang: params.lang,
    slot: params.slot,
    invention_id: entry.id,
    invention_label: entry.itemLabel,
    theme: entry.theme,
    ...
    object_changed: entry.objectChanged,
...
export async function onRequestGet(context: { request: Request; env?: ShareEnv }): Promise<Response> {
  ...
  log("invention_scene_served", {
    year,
    countryQid,
    lang,
    slot,
    inventionId: entry.id,
