...
var init_utils = __esm({
  "../../../.npm/_npx/32026684e21afda6/node_modules/unenv/dist/runtime/_internal/utils.mjs"() {
    init_functionsRoutes_0_5033624626391319();
...
function scoreKeywords(label, keywords) {
  ...
  return keywords.reduce((acc, keyword) => {
    ...
    const kAscii = k2.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    if (kAscii.includes(" ")) {
...
function buildFastSparqlQueryByKeywords(year, countryQid, includeKeywords, limit) {
  const includeFilter = includeKeywords.map((keyword) => `CONTAINS(?articleTitle, "${keyword.toLowerCase()}")`).join(" || ");
  return `
...
function cleanEvent(row) {
  ...
  if (!label && wikipediaUrl) {
    const match2 = wikipediaUrl.match(/\/wiki\/(.+)$/);
    if (match2) {
      label = decodeURIComponent(match2[1]).replace(/_/g, " ");
    }
  }
  return {
...
async function fetchSparqlBindings(query, options) {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    try {
      ...
      return Array.isArray(raw2?.results?.bindings) ? raw2.results.bindings : [];
    } catch (error3) {
      lastError = error3;
      if (attempt < retries) {
...
function buildPlaceHintsQuery(qids) {
...
function parsePlaceHints(bindings) {
  const map = /* @__PURE__ */ new Map();
  for (const row of bindings) {
    const itemQid = extractQid2(row.item?.value);
    if (!itemQid) continue;
    const existing = map.get(itemQid) || {};
    map.set(itemQid, {
      p276Qid: existing.p276Qid || extractQid2(row.p276?.value),
      p276Label: existing.p276Label || row.p276Label?.value || null,
      p131Qid: existing.p131Qid || extractQid2(row.p131?.value),
      p131Label: existing.p131Label || row.p131Label?.value || null,
      p159Qid: existing.p159Qid || extractQid2(row.p159?.value),
      p159Label: existing.p159Label || row.p159Label?.value || null,
      p291Qid: existing.p291Qid || extractQid2(row.p291?.value),
      p291Label: existing.p291Label || row.p291Label?.value || null
    });
  }
  return map;
}
async function enrichPlaceHints(events, timeoutMs) {
  if (!events.length) return events;
  const top = [...events].sort((a, b2) => Number(b2.confidence || 0) - Number(a.confidence || 0)).slice(0, 12);
  ...
  const bindings = await fetchSparqlBindings(query, {
    timeoutMs: Math.min(12e3, timeoutMs),
    retries: 1
  });
  ...
  try {
    ...
    if (mode === "geo") {
      ...
      const stateQuery = buildFastSparqlQueryByKeywords(
        year,
        country,
        ["attentat", "attaque", "explosion", "crash", "inondation", "seisme", "s\xE9isme", "attack", "pandemic", "earthquake"],
        12
      );
      ...
      const chunks = settled.filter((result) => result.status === "fulfilled").map((result) => result.value);
      if (chunks.length === 0) {
        return json8(502, {
    ...
    return json8(200, enriched.slice(0, 20));
  } catch (error3) {
    const errorMessage = error3 instanceof Error ? error3.message : "unknown_error";
...
var init_batch = __esm({
  "api/batch.ts"() {
    ...
    __name(buildFastSparqlQueryByKeywords, "buildFastSparqlQueryByKeywords");
    __name(buildGeoSparqlQuery, "buildGeoSparqlQuery");
    __name(cleanEvent, "cleanEvent");
    __name(fetchSparqlBindings, "fetchSparqlBindings");
    __name(buildPlaceHintsQuery, "buildPlaceHintsQuery");
    __name(parsePlaceHints, "parsePlaceHints");
    __name(enrichPlaceHints, "enrichPlaceHints");
...
function yearFromDate(date) {
  const match2 = String(date || "").match(/^(\d{4})/);
...
var init_openaiPayload = __esm({
  "../src/lib/openaiPayload.ts"() {
    ...
    init_performance2();
    init_ruptureTaxonomy();
    __name(yearFromDate, "yearFromDate");
    __name(buildScenePromptInput, "buildScenePromptInput");
  }
});
...
function resolveFromInfobox(infobox, wikiUrl) {
  ...
  for (const [key, value] of entries) {
    ...
    if (!v) continue;
    if (["lieu", "localisation", "siege", "si\xE8ge", "organisateur"].some((needle) => k2.includes(needle))) {
      const normalizedValue = normalize(v);
      const type = normalizedValue.includes("palais") || normalizedValue.includes("tribunal") || normalizedValue.includes("ministere") || normalizedValue.includes("minist\xE8re") || normalizedValue.includes("assemblee") || normalizedValue.includes("assembl\xE9e") || normalizedValue.includes("conseil") ? "institution" : "site";
      pushCandidate(out, {
        type,
        name: v,
        qid: null,
...
function resolveFromLeadHeuristics(lead, ruptureType, wikiUrl) {
  ...
  const match2 = String(lead || "").match(regex);
  if (match2?.[1]) {
    pushCandidate(out, {
      type: "city",
      name: match2[1].trim(),
...
function resolveFromWikidata(hints, wikiUrl) {
  const out = [];
  if (!hints) return out;
  const siteQid = extractQid3(hints.p276Qid) || extractQid3(hints.p159Qid);
  ...
  const cityQid = extractQid3(hints.p131Qid) || extractQid3(hints.p291Qid);
...
async function resolvePrecisePlace(ctx) {
  ...
  return { selected, candidates, flags: flags2 };
}
var OFFICIAL_INSTITUTIONS;
var init_precisePlaceResolver = __esm({
  "../src/server/place/precisePlaceResolver.ts"() {
    "use strict";
    init_functionsRoutes_0_5033624626391319();
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
    ...
    OFFICIAL_INSTITUTIONS = [
      {
        terms: ["jorf", "journal officiel", "promulgation", "decret", "d\xE9cret", "loi"],
        name: "Journal officiel de la R\xE9publique fran\xE7aise (JORF)",
        sourceUrl: "https://www.legifrance.gouv.fr/"
      },
      {
        terms: ["conseil constitutionnel", "decision", "d\xE9cision"],
        name: "Conseil constitutionnel",
        sourceUrl: "https://www.conseil-constitutionnel.fr/"
      },
      {
        terms: ["assemblee nationale", "assembl\xE9e nationale"],
        name: "Assembl\xE9e nationale",
        sourceUrl: "https://www.assemblee-nationale.fr/"
      },
      {
        terms: ["senat", "s\xE9nat"],
        name: "S\xE9nat",
        sourceUrl: "https://www.senat.fr/"
      },
      {
        terms: ["parlement"],
        name: "Parlement fran\xE7ais",
        sourceUrl: "https://www.vie-publique.fr/"
      }
    ];
    __name(normalize, "normalize");
    __name(isLegalOrState, "isLegalOrState");
    __name(safeName, "safeName");
    __name(extractQid3, "extractQid");
    __name(toCandidateId, "toCandidateId");
    __name(pushCandidate, "pushCandidate");
    __name(resolveFromInfobox, "resolveFromInfobox");
    __name(resolveFromLeadHeuristics, "resolveFromLeadHeuristics");
    __name(resolveFromWikidata, "resolveFromWikidata");
    __name(selectCandidate, "selectCandidate");
    __name(resolvePrecisePlace, "resolvePrecisePlace");
  }
});
...
function isLegalOrState2(ruptureType) {
...
function evaluateStrictPlace(params) {
  ...
  const flags2 = [`date_precision:${datePrecision}`];
  if (!selected) {
    flags2.push("rejected:missing_place");
    return { ok: false, flags: flags2 };
  }
  if (selected.qid && (selected.type === "site" || selected.type === "institution")) {
    flags2.push("accepted:qid_site_or_institution");
    return { ok: true, flags: flags2 };
  }
  if (selected.type === "institution" && hasOfficialSource(sourceUrls)) {
    flags2.push("accepted:institution_with_official_source");
    return { ok: true, flags: flags2 };
  }
  if (selected.type === "city" && isLegalOrState2(ruptureType) && datePrecision === "day") {
    flags2.push("accepted:city_legal_or_state_with_day_precision");
    return { ok: true, flags: flags2 };
  }
  flags2.push("rejected:missing_precise_place");
  return { ok: false, flags: flags2 };
}
var OFFICIAL_DOMAINS;
var init_validateStrictPlace = __esm({
  "../src/server/place/validateStrictPlace.ts"() {
    "use strict";
    init_functionsRoutes_0_5033624626391319();
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
    ...
    __name(hasOfficialSource, "hasOfficialSource");
    __name(isLegalOrState2, "isLegalOrState");
    __name(evaluateStrictPlace, "evaluateStrictPlace");
  }
});

// api/scene.ts
function parseYear6(raw2) {
  const value = Number(raw2);
  return Number.isInteger(value) && value >= 1e3 && value <= 2100 ? value : null;
}
function parseLang(raw2) {
  return String(raw2 || "fr").toLowerCase().startsWith("fr") ? "fr" : "en";
}
function parseQid(raw2) {
  const value = String(raw2 || "").trim().toUpperCase();
  return /^Q\d+$/.test(value) ? value : null;
}
function parseCountryQid(raw2) {
  const value = String(raw2 || "Q142").trim().toUpperCase();
...
function parseMode(raw2) {
  return raw2 === "geo" ? "geo" : "fast";
}
...
function json11(status, payload, headers2 = noStoreHeaders()) {
  return new Response(JSON.stringify(payload), { status, headers: headers2 });
}
async function getFromR2(env2, key) {
  if (!env2.R2) return null;
...
async function putToR2(env2, key, data) {
  if (!env2.R2) {
    throw new Error("R2 binding missing");
  }
  await env2.R2.put(key, JSON.stringify(data), {
    httpMetadata: {
...
function dateToIsoDay(raw2) {
  const match2 = String(raw2 || "").match(/^(\d{4})-(\d{2})-(\d{2})/);
  if (!match2) return null;
...
function validateStrict(params) {
  const flags2 = [...params.placeFlags];
  if (!params.date) {
    flags2.push("date_precision:unknown", "rejected:missing_precise_date");
    return { ok: false, code: "missing_precise_date", message: "date_precision must be day", flags: flags2 };
  }
  if (!params.fact.trim()) {
    flags2.push("rejected:missing_fact");
    return { ok: false, code: "missing_fact", message: "fact required", flags: flags2 };
  }
  if (!params.beforeState.trim() || !params.afterState.trim()) {
    flags2.push("rejected:missing_structural_delta");
    return { ok: false, code: "missing_structural_delta", message: "before_state and after_state required", flags: flags2 };
  }
  if (!params.sources.length) {
    flags2.push("rejected:missing_sources");
    return { ok: false, code: "missing_sources", message: "sources required", flags: flags2 };
  }
  ...
  flags2.push(...placeEvaluation.flags);
  if (!placeEvaluation.ok) {
    return { ok: false, code: "missing_precise_place", message: "place resolver rejected selection", flags: flags2 };
  }
  return { ok: true, flags: flags2 };
}
...
function isBatchItem(value) {
  ...
  const item = value;
  return typeof item.qid === "string" && typeof item.label === "string" && typeof item.date === "string" && typeof item.wikipediaUrl === "string" && typeof item.rupture_type === "string";
}
...
async function buildStableScene(env2, requestUrl, params) {
  ...
  ].filter((entry, index, arr) => arr.findIndex((x2) => x2.url === entry.url) === index);
...
async function handler(context2) {
  if (!context2.env.R2) {
    return json11(500, { error: "missing_r2_binding" });
  }
  const requestUrl = new URL(context2.request.url);
  const year = parseYear6(requestUrl.searchParams.get("year"));
  ...
  if (!year || !countryQid || !eventQid) {
    return json11(400, { error: "Invalid parameters. Expected year,country,qid." });
  }
  const key = r2Key(countryQid, year, eventQid, lang);
  try {
    const fromR2 = await getFromR2(context2.env, key);
    if (fromR2) {
      return json11(200, fromR2, immutableHeaders());
    }
    const prisma = context2.env.__PRISMA_MOCK || getPrismaClient2(context2.env);
    ...
    if (dbRow?.status === "ready" && dbRow?.r2Key) {
      const dbR2 = await getFromR2(context2.env, dbRow.r2Key);
      if (dbR2) {
        return json11(200, dbR2, immutableHeaders());
      }
    }
    if (dbRow?.status === "pending") {
      ...
      if (lockExpiresAt > Date.now()) {
        return json11(202, { status: "pending", event_qid: eventQid });
      }
    }
    ...
    let validationFlags = [];
    try {
      const result = await buildStableScene(context2.env, requestUrl, { year, countryQid, lang, eventQid, mode });
      ...
      if (status === 422) return json11(422, { error: code, message });
      if (status === 404) return json11(404, { error: code, message });
      throw error3;
    }
    await putToR2(context2.env, key, stable);
    await prisma.eventCache.update({
      where: { year_countryQid_lang_eventQid: { year, countryQid, lang, eventQid } },
      data: {
        status: "ready",
    ...
    return json11(200, stable, immutableHeaders());
  } catch (error3) {
    const detail = error3 instanceof Error ? error3.message : "internal_error";
    return json11(500, { error: "scene_request_failed", detail });
  }
}
async function onRequestOptions12() {
  return new Response(null, { status: 204, headers: noStoreHeaders() });
}
var OPENAI_ENDPOINT2, SCHEMA_VERSION, PENDING_TTL_MS, onRequestGet9;
var init_scene = __esm({
  "api/scene.ts"() {
    ...
    init_openaiPayload();
    init_ruptureTaxonomy();
    init_precisePlaceResolver();
    init_validateStrictPlace();
    OPENAI_ENDPOINT2 = "https://api.openai.com/v1/responses";
    SCHEMA_VERSION = "1.0";
    PENDING_TTL_MS = 9e4;
    __name(parseYear6, "parseYear");
    __name(parseLang, "parseLang");
    __name(parseQid, "parseQid");
    __name(parseCountryQid, "parseCountryQid");
    __name(parseMode, "parseMode");
    __name(r2Key, "r2Key");
    __name(noStoreHeaders, "noStoreHeaders");
    __name(immutableHeaders, "immutableHeaders");
    __name(json11, "json");
    __name(getFromR2, "getFromR2");
    __name(putToR2, "putToR2");
    __name(computePromptHash, "computePromptHash");
    __name(fetchJsonWithTimeout, "fetchJsonWithTimeout");
    __name(dateToIsoDay, "dateToIsoDay");
    __name(pickPlace, "pickPlace");
    __name(validateStrict, "validateStrict");
    __name(acquireLock, "acquireLock");
    __name(isBatchItem, "isBatchItem");
    __name(parseOpenAIOutput, "parseOpenAIOutput");
    __name(buildStableScene, "buildStableScene");
    __name(handler, "handler");
    __name(onRequestOptions12, "onRequestOptions");
    onRequestGet9 = handler;
  }
});

// ../.wrangler/tmp/pages-CiQDhv/functionsRoutes-0.5033624626391319.mjs
var routes;
var init_functionsRoutes_0_5033624626391319 = __esm({
  "../.wrangler/tmp/pages-CiQDhv/functionsRoutes-0.5033624626391319.mjs"() {
    "use strict";
    init_events();
    init_events();
  ...
  async fetch(originalRequest, env2, workerContext) {
    ...
    const next = /* @__PURE__ */ __name(async (input, init) => {
      ...
      if (result.done === false) {
        ...
        const response = await handler2(context2);
